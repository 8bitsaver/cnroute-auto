...
from config import DELEGATED_APNIC_DATA_PATH, CNROUTE_V4_DATA_PATH, CNROUTE_V6_DATA_PATH

logging.basicConfig()
logger = logging.getLogger(__name__)

# Only 3 types are includede in the record, so we can only match ipv4/ipv6.
CNROUTE_V4_REGEX = r"apnic\|cn\|ipv4\|.+"
CNROUTE_V6_REGEX = r"apnic\|cn\|ipv6\|.+"
...
def main():
    ...
    if not delegated_apnic_raw_list:
        sys.exit(1)
    ret_v4 = write_chnroute_data(delegated_apnic_raw_list, CNROUTE_V4_REGEX, CNROUTE_V4_DATA_PATH)
    ret_v6 = write_chnroute_data(delegated_apnic_raw_list, CNROUTE_V6_REGEX, CNROUTE_V6_DATA_PATH)
    if not ret_v4:
        logger.error("WRITE CNROUTE v4 DATA FAILED...")
    if not ret_v6:
        logger.error("WRITE CNROUTE V6 DATA FAILED...")
